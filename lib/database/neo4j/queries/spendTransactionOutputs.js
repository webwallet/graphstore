...
const query = `
  ...
  filter(pre in collect(spendee.id + '::' + unspent.id)
    where size(pre) > 0) as previousOutputs
  foreach(unspent in unspents |
...
`
const rest = `
filter(pre in collect(previousOutput) where toBoolean(pre)) as previousOutputs

match (outputsIndex)-[unspent:Unspent]->(spendee:Transaction)
where (spendee.id + '::' + unspent.id) in previous
and not (spendee)<-[:Spends {id: unspent.id}]-()
create (spender)-[spentOutput:Spends {id: unspent.id}]->(spendee)
